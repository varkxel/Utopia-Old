...
namespace Utopia.World {
	...
	public class NoiseMap2D : ScriptableObject {
		...
		// Context data
		[NonSerialized] public NativeArray<double2> octaveOffsets;
		[NonSerialized] public bool octaveOffsetsGenerated;

		public void GenerateOffsets(double range = 64000.0) {
			// Generate offsets by default on creation
			GenerateOffsets(ref Generator.instance.random, true, range);
		}
		...
		public void GenerateOffsets(ref Random random, bool persistent, double range = 64000.0) {
			...
			for (int octave = 0; octave < octaves; octave++) octaveOffsets[octave] = random.NextDouble2(-range, range);
		}
