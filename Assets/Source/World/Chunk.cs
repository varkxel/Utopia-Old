	...
	public class Chunk : MonoBehaviour {
		...
		public void Generate() {
			...
			JobHandle meshDependency =
				JobHandle.CombineDependencies(uvJob, JobHandle.CombineDependencies(indicesJob, vertexJob));
		...
		private JobHandle GenerateBiomes() {
			return Generator.instance.biomes.GenerateChunk
			(
				index, size,
				out biomeMap,
				false
		...
		private struct IndicesJob : IJob {
			...
			*/
			...
			[BurstCompile]
			private static int Pos(int x, int y, int size) {
				return x + y * size;
			}
		}
