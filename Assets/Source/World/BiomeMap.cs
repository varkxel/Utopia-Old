...
namespace Utopia.World {
	...
	public class BiomeMap : ScriptableObject, IDisposable {
		/// <summary>
		...
		/// </summary>
		[Header("Biome List")] public List<Biome> biomes = new List<Biome>();

		/// <summary>
		///     The size of the biome textures.
		/// </summary>
		[Header("Texturing")] public int textureSize = 2048;
		...
		/// </summary>
		[NonSerialized] public Texture2DArray textures;
		...
		public void Dispose() {
			curves.Dispose();
			foreach (Biome biome in biomes) biome.Dispose();
		}
			...
			for (int i = 0; i < biomeCount; i++) {
				...
				if (biomes[i] == null)
					...
					throw new NullReferenceException(
						$"Tried to generate chunk from {nameof(BiomeMap)} \"{name}\" with a null biome set at index {i.ToString()}.");
				#endif
		...
		public void GenerateChunk_OnComplete() {
			...
			foreach (Biome biome in biomes) biome.OnComplete();
		}
		...
		internal struct ModifierJob : IJobParallelFor {
			...
			public void Execute(int index) {
				...
				for (int i = 0; i < 4; i++) samples[i] = Curve.Evaluate((float)heightmap[index], biomeCurves[i]);
